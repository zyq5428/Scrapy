...
from moviejwt.items import MoviejwtItem
...
class MovieSpider(scrapy.Spider):
    ...
    async def parse_detail(self, response):
        ...
        item['name'] = response.css('h2.name::text').extract_first()
        item['tags'] = response.css('.tags span::text').re('[^\x00-\xff]{1,10}')
        price = response.css('.info .price span::text').extract_first()
        ...
        item['authors'] = response.css('.info .authors::text').re('作者：(.*)')[0] \
            if response.css('.info .authors::text').extract_first() else None
        item['published_at'] = response.css('.info .published-at::text').re('(\d{4}-\d{2}-\d{2})')[0] \
            if response.css('.info .published-at::text').extract_first() else None
        item['isbm'] = response.css('.info .isbn::text').re('ISBM：(.*)')[0] \
            if response.css('.info .isbn::text').extract_first() else None
